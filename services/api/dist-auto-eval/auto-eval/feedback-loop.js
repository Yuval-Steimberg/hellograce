 ...
 */
export function loadPreferencePairs(resultsDir) {
 ...
 */
export function pairsToSyntheticFeedback(pairs) {
 ...
 */
export async function evalGateCheck(_llm, _candidatePrompt, baselineScore, logger) {
...
function findWorstCategory(report) {
    let worst = '';
 ...
 */
export async function generateContentRulesFromPatterns(llm, patterns, pool, logger) {
    const actionablePatterns = patterns.filter((p) => p.frequency >= 3 && p.avgScoreImpact >= 1.5);
    ...
    const { rows: existingRules } = await pool.query(`SELECT pattern, reason FROM content_rules WHERE is_active = TRUE`);
...
Generate content rules that would catch these patterns at runtime. Only create rules where a text pattern can reliably detect the issue. Return JSON array only.`,
        ...
        const validRules = rules.filter((r) => {
            if (!r.pattern || !r.reason || !r.severity)
                return false;
            if (r.is_regex) {
                try {
 ...
 */
export async function insertDraftContentRules(pool, rules, logger) {
    let inserted = 0;
    for (const rule of rules) {
        try {
            await pool.query(`INSERT INTO content_rules (rule_type, pattern, is_regex, reason, severity, applies_to, is_active)
         VALUES ($1, $2, $3, $4, $5, 'all', FALSE)`, [rule.rule_type, rule.pattern, rule.is_regex, `[auto-eval] ${rule.reason}`, rule.severity]);
            inserted++;
        }
        catch (err) {
