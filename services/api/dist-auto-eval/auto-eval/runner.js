...
export async function runAutoEval(opts) {
    const logger = pino({ level: 'silent' });
    const simLlm = new GeminiProvider({ apiKey: opts.apiKey, model: opts.model }, logger);
    const evalModel = opts.evaluatorModel ?? opts.model;
    const evalLlm = evalModel !== opts.model
    ...
    for (let i = 0; i < scenarios.length; i += concurrency) {
        ...
        if (opts.verbose) {
            ...
            process.stderr.write(`  [${completed}/${scenarios.length}] simulated${errCount > 0 ? ` (${errCount} errors)` : ''}\n`);
        }
    }
    ...
    for (let i = 0; i < conversations.length; i += concurrency) {
        ...
        if (opts.verbose) {
            ...
            process.stderr.write(`  [${completed}/${conversations.length}] evaluated (batch avg: ${avgScore.toFixed(2)})\n`);
        }
    }
    ...
    const report = buildReport(evaluations, patterns, regressions, suggestions, preferencePairCount, opts.model);
