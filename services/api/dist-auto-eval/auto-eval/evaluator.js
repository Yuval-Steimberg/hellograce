...
function buildTurnEvalPrompt(persona, turns, turnIndex) {
...
function buildConversationLevelPrompt(persona, turns, turnEvals) {
...
export class ConversationEvaluator {
    ...
    async evaluate(conversation) {
        ...
        for (let i = 0; i < userTurnCount; i++) {
            const turnEval = await this.evaluateTurn(conversation.persona, conversation.turns, i);
            turnEvals.push(turnEval);
        }
        const convoLevel = await this.evaluateConversationLevel(conversation.persona, conversation.turns, turnEvals);
    ...
    async evaluateTurn(persona, turns, turnIndex) {
        const userTurnIdx = turnIndex * 2;
        const graceTurnIdx = userTurnIdx + 1;
    ...
    async evaluateConversationLevel(persona, turns, turnEvals) {
    ...
    defaultTurnEval(turnIndex, userMessage, graceResponse) {
        return {
            turnIndex,
            userMessage,
            graceResponse,
