...
export function buildMultiPartNote(understanding: MessageUnderstanding): string {
  ...
  return (
    '\n\nThe user just said a few things in one text. Reply to ALL of it, the way a friend texts back. LEAD WITH THE ANSWER — do NOT open by narrating what you\'re about to do ("let\'s break down", "let\'s discuss", "here\'s how", "estimating protein from…"), do NOT restate/label/analyze their message, and do NOT write a heading. React to any feeling OR how they say they\'re doing physically (hungry, tired, low energy, stressed) FIRST in a few warm words. Then give EACH thing its own short paragraph (2-4 sentences), separated by a BLANK LINE, so every part is clearly covered and easy to read — but keep it warm, flowing prose, NOT a report: NO numbers, NO bullets, NO "1./2.", NO headings or labels before a paragraph. For a food they named, COMMIT to a rough protein/calorie number or range (don\'t hedge with "it\'s tough to say" — just estimate and say it\'s approximate); for what to eat next, name 1-2 concrete foods. Use ONLY what THIS message says; never bring in a food or topic from earlier turns.'
  );
}
