...
async function buildServer(): Promise<{ app: FastifyInstance; shutdown: () => Promise<void> }> {
  ...
  logger.info(
    { unifiedReplyPath: env.UNIFIED_REPLY_PATH, geminiFirst: env.GEMINI_FIRST, directReplyMode: env.DIRECT_REPLY_MODE, leanReplyMode: env.LEAN_REPLY_MODE, compactReplyMode: env.COMPACT_REPLY_MODE },
    env.UNIFIED_REPLY_PATH
      ? 'startup.unified_reply_path — Nudge-style single grounded reply with Grace tools and safety floors'
  ...
  if (env.OPTIMIZERS_ENABLED) {
    ...
    logger.info('research.auto_fix.disabled');
  }
  ...
  registerSettingsRoutes(app, {
    ...
    localTestMode: env.LOCAL_TEST_MODE && env.NODE_ENV !== 'production',
  });
