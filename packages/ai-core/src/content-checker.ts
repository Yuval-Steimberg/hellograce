...
const BANNED_PHRASES: Array<{ pattern: RegExp; reason: string }> = [
  ...
  { pattern: /\bof\s+course\s+[—\-,]\s+what\s+works\s+better\s+for\s+you\b/i, reason: 'Settings-question deflection — should redirect to grace-admin-git-main-yuval-steimbergs-projects.vercel.app/settings, not ask' },
  ...
  { pattern: /\bjust tell me the new date\b.{0,30}(treatment|start)/i, reason: 'no natural-language date-edit tool; direct user to grace-admin-git-main-yuval-steimbergs-projects.vercel.app/settings' },
];
 ...
 * The prompt requires Grace to emit the real URL (https://grace-admin-git-main-yuval-steimbergs-projects.vercel.app/settings).
 */
export function checkLinkPlaceholder(text: string): ContentViolation[] {
  if (/\[(link|settings link|url|here)\]/i.test(text)) {
    return [{
      ...
      message: 'emitted a "[link]" placeholder instead of the real URL https://grace-admin-git-main-yuval-steimbergs-projects.vercel.app/settings',
      match: text.match(/\[[^\]]+\]/)?.[0],
    }];
  }
  if (/<link>/i.test(text)) {
...
export function buildContentRegenInstruction(
  ...
  if (linkHits.length > 0) {
    parts.push(
      `Your draft used a "[link]" placeholder. Replace it with the literal URL https://grace-admin-git-main-yuval-steimbergs-projects.vercel.app/settings — never write a placeholder.`,
    );
  }
