...
const REQUIRED_BEHAVIOR_PHRASES = [
  'BANNED',
  'grace-admin-git-claude-gemini-c34cfe-yuval-steimbergs-projects.vercel.app/settings',
  'GLP-1',
];
...
export class PromptOptimizer {
  ...
  private async runWithLock(_client: import('pg').PoolClient): Promise<void> {
    ...
    const saveResult = await this.saveVersion(newPrompt, mergeAnalysis, safe);
    const version = saveResult.version;
    // `actuallyActivated` reflects what made it into the DB. If saveVersion
    // tripped the unique-violation fallback path, the prompt went in as an
    // inactive DRAFT even though we asked for activation.
    const actuallyActivated = saveResult.activated;
    ...
    if (actuallyActivated) {
      ...
      this.logger.warn(
        { reason: mergeAnalysis, requestedActivation: safe, gotActivation: false },
        'prompt_optimizer.saved_as_draft',
      );
    }

    // If we asked for activation but got a draft, surface the race in the
    // report so the admin sees it without searching logs.
    const draftReason = actuallyActivated
      ? undefined
      : safe
        ? 'Concurrent optimizer run took the active slot first — saved as DRAFT instead of crashing on the unique constraint. Open /admin/prompts to review.'
        : 'Safety or eval gate failed. The prompt may have dropped required phrases or scored below the auto-eval baseline. Saved as draft for manual review.';

    await this.emitReport({
      status: actuallyActivated ? 'activated' : 'draft',
...
Respond with ONLY the JSON object.`,
  ...
  private async saveVersion(content: string, analysis: string, autoActivate: boolean): Promise<{ version: number; activated: boolean }> {
    ...
    if (autoActivate) {
      // Defense-in-depth against the duplicate-key crash:
      //
      //   1. Real transaction on a single checked-out client. The previous
      //      pool.query('BEGIN') pattern was a no-op because each pool.query
      //      grabbed a different connection — UPDATE and INSERT were
      //      autocommitting independently.
      //
      //   2. SELECT FOR UPDATE on the current active prompt SERIALIZES the
      //      activation swap at the row level. Two concurrent transactions
      //      can no longer both reach the INSERT step — the second one
      //      blocks on the row lock until the first commits, then sees the
      //      committed swap and can react accordingly.
      //
      //   3. If the unique violation still fires somehow (e.g. another
      //      transaction inserted active=TRUE on a row that DIDN'T exist
      //      when we took the SELECT FOR UPDATE), catch it and gracefully
      //      fall back to inactive-draft instead of crashing. The optimizer
      //      report will then show "saved as draft" instead of "crashed".
      //
      // Codes 23505 = unique_violation in PostgreSQL.
      const client = await this.pool.connect();
      try {
        await client.query('BEGIN');
        // (2) Take a row-level lock on any currently-active prompt so two
        //     concurrent activation swaps serialize cleanly.
        await client.query(`SELECT id FROM prompts WHERE active = TRUE FOR UPDATE`);
        ...
        return { version: nextVersion, activated: true };
      } catch (err) {
        await client.query('ROLLBACK').catch(() => undefined);
        // (3) Detect duplicate-key violations on the active-uniqueness index.
        //     Fall back to inserting as inactive draft so the optimizer
        //     report shows "draft" instead of crashing.
        const pgErr = err as { code?: string; constraint?: string };
        const isUniqueViolation =
          pgErr?.code === '23505' &&
          (pgErr.constraint === 'prompts_active_unique' ||
            String(err).includes('prompts_active_unique'));
        if (isUniqueViolation) {
          this.logger.warn(
            { nextVersion, constraint: pgErr.constraint },
            'prompt_optimizer.unique_violation_fallback_to_draft',
          );
          // Bump the version once more in case the racing run took our slot.
          const { rows: maxRows } = await this.pool.query<{ max: number | null }>(
            `SELECT MAX(version) AS max FROM prompts`,
          );
          const draftVersion = (maxRows[0]?.max ?? nextVersion) + 1;
          await this.pool.query(
            ...
            [
              draftVersion,
              content,
              `${notes} — saved as DRAFT (another run activated a prompt first)`,
            ],
          );
          return { version: draftVersion, activated: false };
        }
        throw err;
      } finally {
        client.release();
      ...
      return { version: nextVersion, activated: false };
    }
  }
}
