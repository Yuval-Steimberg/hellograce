import { ToolRegistry, type Tool } from '@grace/ai-core';
 ...
 * separately.
 */
export function buildMockToolRegistry(): ToolRegistry {
  const registry = new ToolRegistry();
  for (const tool of MOCK_TOOLS) registry.register(tool);
  return registry;
}
  ...
  },
  {
    ...
    execute: async (args) => ({
      results: [
        {
          content:
            'GLP-1 medications can cause GI side effects including nausea, constipation, and reduced appetite. Always consult your prescriber for personal guidance.',
          score: 0.82,
        },
      ],
      args,
