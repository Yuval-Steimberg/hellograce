...
export function buildReport(
  ...
  const passedTurns = evaluations.reduce(
    (sum, e) => sum + e.turnEvaluations.filter((t) => t.passed).length,
    0,
  );
...
function buildCategoryScores(
  evaluations: ConversationEvaluation[],
): Record<string, { avg: number; count: number; passRate: number; worstScore: number }> {
  const byCategory = new Map<ScenarioCategory, ConversationEvaluation[]>();
  ...
  const result: Record<string, { avg: number; count: number; passRate: number; worstScore: number }> = {};
  for (const [cat, evals] of byCategory) {
    const scores = evals.map((e) => e.overallScore);
    const passed = evals.reduce(
      (sum, e) => sum + e.turnEvaluations.filter((t) => t.passed).length,
      0,
    );
...
function buildDimensionScores(
  evaluations: ConversationEvaluation[],
): Record<string, { avg: number; failRate: number }> {
  const byDim = new Map<EvalDimensionName, number[]>();
  for (const e of evaluations) {
  ...
  const result: Record<string, { avg: number; failRate: number }> = {};
...
export function formatReport(report: AutoEvalReport): string {
  const lines: string[] = [];
  ...
  for (const [cat, data] of cats) {
    ...
    lines.push(
      `  ${cat.padEnd(22)} ${bar} ${data.avg.toFixed(2)}  pass:${pct(data.passRate).padStart(4)}  worst:${data.worstScore.toFixed(2)}  (n=${data.count})`,
    );
  }
  lines.push('');
  ...
  for (const [dim, data] of dims) {
    const bar = scoreBar(data.avg);
    lines.push(
      `  ${dim.padEnd(28)} ${bar} ${data.avg.toFixed(2)}  fail:${pct(data.failRate).padStart(5)}`,
    );
  }
  lines.push('');
  ...
  if (report.regressions.length > 0) {
    ...
    for (const reg of report.regressions) {
      ...
      lines.push(
        `  ${icon} ${reg.dimension}: ${reg.previousAvg.toFixed(2)} -> ${reg.currentAvg.toFixed(2)} (${reg.delta > 0 ? '+' : ''}${reg.delta.toFixed(2)})`,
      );
    }
    lines.push('');
  }
