...
const CONSUMPTION_RE: RegExp[] = [
  ...
  /^\s*(?:i\s+)?(?:just\s+|only\s+|already\s+|also\s+|finally\s+|recently\s+|earlier\s+|simply\s+|merely\s+|actually\s+|honestly\s+|basically\s+|literally\s+|kinda\s+|sorta\s+)*(?:ate|eaten|had|have\s+had|have\s+eaten|drank|sipped|finished|finished\s+eating|done\s+eating|polished\s+off|demolished|devoured|scarfed(?:\s+down)?|inhaled|downed|chowed(?:\s+down)?|wolfed(?:\s+down)?|gobbled(?:\s+up)?|snacked(?:\s+on)?|grabbed|consumed)\b/i,
  ...
  /\bi\s+(?:just\s+|only\s+|already\s+|also\s+|finally\s+|recently\s+|earlier\s+|simply\s+|merely\s+|actually\s+|honestly\s+)*(?:ate|had|drank|sipped|finished\s+eating|finished\s+(?:my|the|a)|polished\s+off|devoured|scarfed|downed|consumed|grabbed)\b/i,
