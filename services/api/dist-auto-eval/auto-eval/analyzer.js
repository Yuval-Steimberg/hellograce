export function analyzeResults(evaluations, previousReport) {
...
function detectRegressions(evaluations, previousReport) {
    const regressions = [];
...
function generateSuggestions(patterns, evaluations) {
    const suggestions = [];
    ...
    const safeFallbackCount = evaluations.reduce((count, e) => {
        for (const te of e.turnEvaluations) {
            const hasFallback = te.weaknesses.some((w) => w.toLowerCase().includes('fallback') || w.toLowerCase().includes('generic'));
            if (hasFallback)
                count++;
        }
        return count;
    ...
    if (totalTurns > 0 && safeFallbackCount / totalTurns > 0.1) {
        suggestions.push(`Safe fallback rate is ${((safeFallbackCount / totalTurns) * 100).toFixed(1)}% — investigate why the orchestrator fails to generate valid responses.`);
    }
    return suggestions;
}
